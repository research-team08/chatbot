...
function getRoutineCachePath() {
  return path.join(__dirname, ".routine-cache.json");
}

function readRoutineCache() {
  try {
    const filePath = getRoutineCachePath();
...
function writeRoutineCache(cache) {
  try {
    fs.writeFileSync(getRoutineCachePath(), JSON.stringify(cache, null, 2), "utf-8");
  } catch (err) {
    console.error("Failed to write routine cache:", err.message);
  }
}

function detectRoutineChange(cacheKey, data) {
  const cache = readRoutineCache();
  const key = String(cacheKey || "routine").toLowerCase();
  const currentSignature = JSON.stringify(data);
  const previousSignature = cache[key]?.signature || "";
  const previousUpdatedAt = cache[key]?.updatedAt || "";
  const hasPrevious = Object.prototype.hasOwnProperty.call(cache, key);
  const changed = hasPrevious && previousSignature !== currentSignature;
  const previousCount = cache[key]?.count || 0;
...
const server = http.createServer(async (req, res) => {
  ...
  // ── Backend root (no frontend) ────────────────
  if (parsedUrl.pathname === "/" && req.method === "GET") {
    return sendJSON(res, 200, {
      app: "planner_ai",
      mode: "backend-only",
      message: "Backend is running. Use /api/status, /api/tasks, /api/routine, /api/run.",
    });
  }
